...
const applicant1 = {
    ...
    address: { state: 'MD', city: 'Baltimore', zipCode: 21201 }
};
...
let isApplicant1Active = applicant1.programStatus === 'Active' ? true : false;;
...
const filterApplicantsByEligibility = (applicants) => {
   ...
   for(let i = 0; i < applicants.length; i++) {
    if (applicants[i].age >= 18) {
        eligibleApplicants.push(applicants[i]);
    }
   }
...
const filterApplicantsByProgramStatus = (applicants) => {
  let activeApplicants = [];
   for(let i = 0; i < applicants.length; i++) {
    if (applicants[i].programStatus === 'Active') {
